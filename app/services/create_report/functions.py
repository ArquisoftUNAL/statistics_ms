...
async def create_empty_habit_measure_report() -> rm.HabitMeasureReport:
...
async def create_empty_habit_yn_report() -> rm.HabitYNReport:
...
async def day_progress(df: pd.DataFrame, goal: int, today: date) -> rm.DataResume:
...
async def week_progress(
    data_frame: pd.DataFrame, goal: int, today: date, freq_type: int
) -> rm.DataResume:
...
async def semester_progress(
    df: pd.DataFrame, goal: int, today: date, freq_type: int
) -> rm.DataResume:
    year, *_ = today.isocalendar()
    month = today.month
...
async def ms_day_history(df: pd.DataFrame) -> rm.ListDateValue:
    df = df[["hab_dat_collected_at", "hab_dat_amount"]]
    list_date = df.apply(
        lambda row: rm.DateValue(
            year=row["hab_dat_collected_at"].year,
            month=row["hab_dat_collected_at"].month,
            day=row["hab_dat_collected_at"].day,
            value=row["hab_dat_amount"],
        ),
        axis=1,
    ).to_list()
...
async def ms_week_history(df: pd.DataFrame) -> rm.ListDateValue:
    df = df[["hab_dat_collected_at", "hab_dat_amount", "year", "week"]]
    df = df.groupby(["year", "week"])["hab_dat_amount"].sum().to_frame()
    df.reset_index(inplace=True)
    list_date = df.apply(
        lambda row: rm.DateValue(
            year=row["year"], week=row["week"], value=row["hab_dat_amount"]
        ),
        axis=1,
    ).to_list()
    return rm.ListDateValue(data=list_date)
...
async def ms_month_history(df: pd.DataFrame) -> rm.ListDateValue:
    df = df[["hab_dat_collected_at", "hab_dat_amount", "year", "month"]]
    df = df.groupby(["year", "month"])["hab_dat_amount"].sum().to_frame()
    df.reset_index(inplace=True)
    list_date = df.apply(
        lambda row: rm.DateValue(
            year=row["year"], month=row["month"], value=row["hab_dat_amount"]
        ),
        axis=1,
    ).to_list()
    return rm.ListDateValue(data=list_date)
...
async def ms_semester_history(df: pd.DataFrame) -> rm.ListDateValue:
    df = df[["hab_dat_collected_at", "hab_dat_amount", "year", "month"]]
    ...
    list_date = df.apply(
        lambda row: rm.DateValue(
            year=row["year"], semester=row["semester"], value=row["hab_dat_amount"]
        ),
        axis=1,
    ).to_list()
    return rm.ListDateValue(data=list_date)
...
async def ms_year_history(df: pd.DataFrame) -> rm.ListDateValue:
    df = df[["hab_dat_collected_at", "hab_dat_amount", "year"]]
    df = df.groupby(df["year"])["hab_dat_amount"].sum().to_frame()
    df.reset_index(inplace=True)
    list_date = df.apply(
        lambda row: rm.DateValue(year=row["year"], value=row["hab_dat_amount"]), axis=1
    ).to_list()
    return rm.ListDateValue(data=list_date)


# Functions for get_habit_yn_resume report:
async def month_yn_resume(df: pd.DataFrame, freq: int, today: date) -> float:
    year = today.year
    month = today.month
    month_days = calendar.monthrange(year, month)[1]
    goal = 0
...
async def semester_yn_resume(df: pd.DataFrame, freq: int, today: date) -> float:
    year = today.year
    month = today.month
    goal = 0
    if freq == 1:
...
async def year_yn_resume(df: pd.DataFrame, freq: int, today: date) -> float:
    year = today.year
    month = today.month
    goal = 0
    if freq == 1:
...
async def total_yn_resume(df: pd.DataFrame, today: date) -> int:
    df = df[df["year"] == today.year]
    return df.shape[0]


# Functions for get_habit_yn_history report:
async def yn_week_history(df: pd.DataFrame) -> rm.ListDateCount:
    df = df[["hab_dat_collected_at", "year", "week"]].rename(
        columns={"hab_dat_collected_at": "count"}
    )
    df = df.groupby(["year", "week"]).count()
    df.reset_index(inplace=True)
    list_date = df.apply(
        lambda row: rm.DateCount(
            year=row["year"], week=row["week"], count=row["count"]
        ),
        axis=1,
    ).to_list()
...
async def yn_month_history(df: pd.DataFrame) -> rm.ListDateCount:
    df = df[["hab_dat_collected_at", "year", "month"]].rename(
        columns={"hab_dat_collected_at": "count"}
    )
...
async def yn_semester_history(df: pd.DataFrame) -> rm.ListDateCount:
    df = df[["hab_dat_collected_at", "year", "month"]].rename(
        columns={"hab_dat_collected_at": "count"}
    )
    df["semester"] = df["month"].apply(lambda x: 1 if x <= 6 else 2)
    ...
    list_date = df.apply(
        lambda row: rm.DateCount(
            year=row["year"], semester=row["semester"], count=row["count"]
        ),
        axis=1,
    ).to_list()
    return rm.ListDateCount(data=list_date)
...
async def yn_year_history(df: pd.DataFrame) -> rm.ListDateCount:
    df = df[["hab_dat_collected_at", "year"]].rename(
        columns={"hab_dat_collected_at": "count"}
    )
    df = df.groupby("year").count()
    df.reset_index(inplace=True)
    list_date = df.apply(
        lambda row: rm.DateCount(year=row["year"], count=row["count"]), axis=1
    ).to_list()
    return rm.ListDateCount(data=list_date)


# Functions for get_habit_yn_best_streak report:
async def yn_streaks(df: pd.DataFrame, freq: int) -> rm.ListHabitStreak:
...
async def ms_streaks(df: pd.DataFrame, freq: int) -> rm.ListHabitStreak:
    freq = freq_streaks[freq]
...
async def freq_week_day(df: pd.DataFrame) -> rm.ListHabitFreqWeekDay:
    df = df[["year", "month", "weekday", "hab_dat_amount"]]
    df = df.groupby(["year", "month", "weekday"]).count()
    df.reset_index(inplace=True)
    list_data = df.apply(
        lambda row: rm.HabitFreqWeekDay(
            year=row["year"],
            month=row["month"],
            week_day=row["weekday"],
            quantity=row["hab_dat_amount"],
        ),
        axis=1,
    ).to_list()
