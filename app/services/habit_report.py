...
class HabitReport:
    ...
    async def get_habit_measure_report(self, hab_id: UUID) -> md.HabitMeasureReportModel:
        ...
        return md.HabitMeasureReportModel(
            ...
            streaks = await self.get_habit_ms_streaks(habit_data),
            days_frequency = await self.get_habit_freq_week_day(habit_data)
        )
    ...
    async def get_habit_ms_streaks(self, habit_data: HabData = None, hab_id: UUID = None) -> md.HabitMSStreakReportModel:
        """
        ...
        Returns:
            HabitMSBestStreakReportModel: The report with the best streak of the habit.
        """
        if habit_data == None:
            habit_data = await self.repo.get_habit_data(hab_id)
